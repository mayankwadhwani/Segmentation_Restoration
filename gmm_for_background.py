...
if __name__ == "__main__":
	...
	if not options.clusters :
		k = 4
		print("Used default number of clusters = %d"%k )
	else: k = int(options.clusters)

	if not options.epsilon :
		eps = 0.0001
		print("Used default eps = %f"%eps )
	else: eps = float(options.epsilon)

	if not options.max_iters :
		max_iters=100
		print("Used default maxiters = %d"%max_iters)
	...
	for i in range(Y[:,0].size-1):
		q=np.argmax(gmm.predict(np.array(Y[i],dtype='float32')),axis=0)
		count[q]+=1	
	'''
	...
	Z=Y
	for i in range(Y[:,0].size-1):
		q=np.argmax(gmm.predict(np.array((Y[i,1:3]),dtype='float32')),axis=0)
		if(q!=0):
			Z[i,1:3]=[np.random.normal(gmm.params.mu[0][0] ), np.random.normal(
	Z1=np.reshape(Z,p)
	print(Z1.shape)
	Z2=cv.cvtColor(Z1,cv.COLOR_YCR_CB2BGR)
	cv.imwrite("tmp.jpg",Z2)
	"""
